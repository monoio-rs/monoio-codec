// Modified tokio's implementation to use monoio io traits natively
...
//! # Decoding
//!
//! [`FramedRead`] adapts an [`AsyncReadRent`] into a `Stream` of [`BytesMut`],
...
//! # use monoio_codec::length_delimited::LengthDelimitedCodec;
//! # fn bind_read<T: AsyncReadRent>(io: T) {
...
//! # Encoding
//!
//! [`FramedWrite`] adapts an [`AsyncWriteRent`] into a `Sink` of [`BytesMut`],
...
//! # use monoio_codec::length_delimited::LengthDelimitedCodec;
//! # fn write_frame<T: AsyncWriteRent>(io: T) {
//! # let _ =
//! LengthDelimitedCodec::builder()
//!     .length_field_type::<u16>()
//!     .new_write(io);
...
//! [`AsyncReadRent`]: trait@monoio::io::AsyncReadRent
//! [`AsyncWriteRent`]: trait@monoio::io::AsyncWriteRent
...
impl Builder {
    ...
    ///
    /// # fn bind_read<T: AsyncReadRent>(io: T) {
    /// LengthDelimitedCodec::builder().big_endian().new_read(io);
    /// # }
    /// # pub fn main() {}
    /// ```
    ...
    ///
    /// # fn bind_read<T: AsyncReadRent>(io: T) {
    /// LengthDelimitedCodec::builder().little_endian().new_read(io);
    /// # }
    /// # pub fn main() {}
    /// ```
    ...
    ///
    /// # fn bind_read<T: AsyncReadRent>(io: T) {
    /// LengthDelimitedCodec::builder().native_endian().new_read(io);
    /// # }
    /// # pub fn main() {}
    /// ```
    ...
    ///
    /// # fn bind_read<T: AsyncReadRent>(io: T) {
    /// LengthDelimitedCodec::builder().num_skip(4).new_read(io);
    /// # }
    /// # pub fn main() {}
    /// ```
    ...
    /// # use monoio::io::{AsyncReadRent, AsyncWriteRent};
    /// # use monoio_codec::length_delimited::LengthDelimitedCodec;
    /// # fn write_frame<T: AsyncReadRent + AsyncWriteRent>(io: T) {
    /// # let _ =
    /// LengthDelimitedCodec::builder()
    ///     .length_field_type::<u16>()
    ///     .new_framed(io);
